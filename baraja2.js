...
document.getElementById('baraja2-button').addEventListener('click', function() {
    if (selectedCard2) {
        fetchAndRevealSelectedPokemon('pokedex2', selectedCard2, 'baraja2-button');
    } else {
        mostrarError('Jugador 2 debe seleccionar una carta antes de barajar.');
    }
});
...
function fetchAndRevealSelectedPokemon(containerId, selectedCard, buttonId) {
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            // Después de revelar, deshabilitar el botón 'Barajar'
            document.getElementById(buttonId).disabled = true;

            // Verificar si Jugador 1 ya ha revelado su carta
            if (selectedCard1 && selectedCard1.getAttribute('data-power')) {
                compararPokemon();
            }
        })
        .catch(error => {
